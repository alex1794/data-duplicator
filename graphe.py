...
def dataread(filename):
    ...
    return data, head_array, mean_array, time_yerr, bw_yerr
...
read_list = glob("bs_file*", recursive=False)
...
if glob("bs_c1.txt"):
    for filename, labels in filelist.items():
        data, head_array, mean_array, time_yerr, bw_array = dataread(filename)
...
if glob("bs_file_4.txt"):
    for filename in read_list:
        data, head_array, mean_array, time_yerr, bw_yerr = dataread(filename)

        fig = plt.figure(figsize=(15,12))
        plt.errorbar(head_array[:,1].astype(float), mean_array[:,1], yerr=time_yerr, ecolor="red", elinewidth=1.0, capsize=2.0, capthick=1.0)
        #plt.boxplot(data[:,1:,1].astype(float).transpose()*1e-6, widths=0.5)
        plt.xscale('log')
        plt.xticks(head_array[:,1].astype(float), xlab1)
        #plt.xticks(np.arange(1, len(mean_array)+1), xlab1)
        plt.xlabel("Blocksize [B]")
        plt.ylabel("Time [s]")
        plt.title("Reading time of "+filename[3:-4]+" in function of block size")
        plt.savefig("time_"+filename[:-4]+".png")
        plt.close()
        
        fig = plt.figure(figsize=(15,12))
        plt.errorbar(head_array[:,1].astype(float), mean_array[:,2]*1e-9, yerr=bw_yerr*1e-9, ecolor="red", elinewidth=1.0, capsize=2.0, capthick=1.0)
        #plt.boxplot(data[:,1:,2].astype(float).transpose()*1e-9, widths=0.5)
        plt.xscale('log')
        plt.xticks(head_array[:,1].astype(float), xlab1)
        #plt.xticks(np.arange(1, len(mean_array)+1), xlab1)
        plt.xlabel("Blocksize [B]")
        plt.ylabel("Bandwidth [GB/s]")
        plt.title("Reading bandwidth of "+filename[3:-4]+" in function of block size")
        plt.savefig("bw_"+filename[:-4]+".png")
        plt.close()
